...
在更多时候，我们希望构建的Agents是proactive的：Agent可以持续地、在真实时间线上一直运行，它可以：
...
在这种逻辑下，Agent就不再是按照Request-Response的方式被构建，而是需要一个基于循环和状态的 Agent 系统。

「触发」（Trigger），也可以认为是「定时」（scheduler）成为了Agent最重要的能力。
...
| :--- | :--- | :--- |
| 突发事件触发 (Event-Driven) | 重要的、无法预测的外部信息突然出现。此前的user query 就可以理解为一类特殊的 event。 | 1. 突发新闻：某公司 CEO 辞职、央行降息公告。<br>2. 新任务发布：预测系统发布了一个新的预测挑战。 |
...
## Self-Scheduling/Subscribe

“自主调度”是实现proactive agent的一种形式，它要求 Agent 不仅能对外界信息做出反应，还能根据推理安排未来的行动。
...
订阅则是允许LLM像agentic 操作memory一样操作订阅信息：
